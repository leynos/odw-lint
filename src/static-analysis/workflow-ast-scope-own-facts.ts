/**
 * @file Own binding facts for lexical workflow AST scopes.
 *
 * This module owns scope-local declaration collection for the deterministic
 * scanner family. Callers should consume `rootScopeOwnFacts` or
 * `scopeOwnFacts` rather than duplicating SWC scope-boundary traversal.
 */
...
const collectScopeParameters = (scopeNode: AstNode, boundNames: Set<string>): void => {
  ...
  collectFunctionParamBindings(
    ...
    boundNames,
    (child) =>
      // Parameter-pattern recursion may see default initializers, but those
      // initializers are not declarations owned directly by this scope
      // boundary. Keep their names, and discard alias candidates deliberately.
...
const collectPatternBindingNames = (pattern: unknown, boundNames: Set<string>): void => {
  collectPatternBindings(
    ...
    boundNames,
    (child) =>
      // Destructuring recursion shares the same name collector from variable
      // declarations, parameters and catches. Alias initializers are only valid
      // for direct declarators, so nested pattern traversal drops them.
