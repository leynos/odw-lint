...
This rule reports when the static parser can prove that `meta` is not an object
in the supported static contract. A workflow cannot be loaded when its metadata
is a primitive, array, function, or other unsupported value.
