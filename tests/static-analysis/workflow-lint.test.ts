...
describe("lintWorkflowSource", () => {
  ...
  it("routes chained ODW-only validate aliases through the merged pipeline", () => {
    const sourceText = [
      "export const meta = { name: 'example', description: 'ok' };",
      "const v = validate;",
      "const w = v;",
    ...
    expect(diagnostic).toBeDefined();
    if (diagnostic === undefined) {
      throw new Error("expected one chained ODW-only validate alias diagnostic");
    }

    expect(diagnostic.severity).toBe("info");
    expect(diagnostic.message).toBe(firstReviewedRuleMessage(rule));
    expect(diagnostic.docs).toBe("docs/rules/no-odw-only-validate.md");
    expectSpanToMatchSource(result.sourceFile.sourceText, diagnostic.span, "w");
  });
